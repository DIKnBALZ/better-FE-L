...
class OriginalChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var characters:Array<String> = sys.FileSystem.readDirectory("assets/characters");
	...
	// yoshubs im going to brutally stab and murder you
		...
		if (!typingShit.hasFocus) {
			...
			if (!FlxG.keys.pressed.SHIFT) { // i can work on this maybe
				if (FlxG.keys.pressed.W || FlxG.keys.pressed.S) {
					songMusic.pause();
					vocals.pause();

					var daTime:Float = 700 * FlxG.elapsed;
					if (FlxG.keys.pressed.W) songMusic.time -= daTime;
					else songMusic.time += daTime;
					vocals.time = songMusic.time;
				}
			} else {
				if (FlxG.keys.justPressed.W || FlxG.keys.justPressed.S) {
					...
					if (FlxG.keys.justPressed.W) songMusic.time -= daTime;
					else songMusic.time += daTime;
					vocals.time = songMusic.time;
				}
			}
	...
	function deleteNote(note:Note):Void {
		...
		if (data > -1) {
			for (i in _song.notes[curSection].sectionNotes) { // can i remove this brace idk lmao
	...
	private function addNote():Void {
		...
		if (FlxG.keys.pressed.CONTROL) _song.notes[curSection].sectionNotes.push([noteStrum, (noteData + 4) % 8, noteSus, noteType]);
	...
	private function saveLevel() {
		var json = {"song": _song};
		var data:String = Json.stringify(json, "\t"); // oo fancy B)
	...
	function onSaveCancel(_):Void { // Called when the save file dialog is cancelled.
		_file.removeEventListener(Event.COMPLETE, onSaveComplete);
		_file.removeEventListener(Event.CANCEL, onSaveCancel);
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
		_file = null;
	...
	function onSaveError(_):Void { // Called if there is an error while saving the gameplay recording.
		_file.removeEventListener(Event.COMPLETE, onSaveComplete);
		_file.removeEventListener(Event.CANCEL, onSaveCancel);
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
		_file = null;
