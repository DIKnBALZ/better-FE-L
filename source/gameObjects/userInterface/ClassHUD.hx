...
class ClassHUD extends FlxTypedGroup<FlxBasic>
{
	...
	var engineDisplay:String = "FE v" + Main.gameVersion + " // " + Main.gameBetterVersion;
	...
	public function new() {
		...
		if (Init.trueSettings.get('Downscroll')) centerMark.y = (FlxG.height - centerMark.height / 2) - 30;
		else centerMark.y = (FlxG.height / 24) - 10;
		...
		if (Init.trueSettings.get('Counter') != 'None') {
			...
			for (i in 0...judgementNameArray.length) {
				var textAsset:FlxText = new FlxText(5 + (!left ? (FlxG.width - 10) : 0), (FlxG.height / 2) - (counterTextSize * (judgementNameArray.length / 2)) + (i * counterTextSize), 0, '', counterTextSize);
	...
	override public function update(elapsed:Float) {
		...
		for (i in [iconP1, iconP2]) {
			i.setGraphicSize(Std.int(FlxMath.lerp(i.initialWidth, i.width, iconLerp)));
			i.updateHitbox();
		}
